...
export class UserActivityService {
  ...
  async handleUserLoggedInEvent({ payload }: UserLoggedInEvent) {
    ...
    try {
      await this.entityManger.transaction(async (txEntityManger: EntityManager) => {
        const { createdAt } = await txEntityManger.getRepository(UserActivity).save({
          event: UserLoggedInEvent.eventName,
          userId,
          meta,
        });

        await txEntityManger.createQueryBuilder()
          .update(User)
          .where({ id: userId })
          .set({
